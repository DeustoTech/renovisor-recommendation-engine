# fichero para juntar por cada escenario los individuales + greedy en un 2x2
library(magick)
...
# 32
# base_ind <- "results/det32/results_each_archetype_separate/by_scenario"
# base_greedy <- "results/det32/results_greedy_from_freq/by_scenario"
# out_dir <- "results/det32/final_compare_pdfs"

# 9
# base_ind <- "results/det9/results_each_archetype_separate_9/by_scenario"
# base_greedy <- "results/det9/results_greedy_from_freq_9/by_scenario"
# out_dir <- "results/det9/final_compare_pdfs_9"

# 9 top3data
base_ind <- "results/det9_top3/results_each_archetype_separate_9_top3/by_scenario"
base_greedy <- "results/det9_top3/results_greedy_from_freq_9_top3/by_scenario"
out_dir <- "results/det9_top3/final_compare_pdfs_9_top3"
...
for (tp in types) {
  for (sc in scenarios) {
    ...
    file_experts <- file.path(base_ind, paste0("individual_", tp, "_", sc, "_experts.pdf"))
    file_kmeans  <- file.path(base_ind, paste0("individual_", tp, "_", sc, "_kmeans.pdf"))
    file_sins    <- file.path(base_ind, paste0("individual_", tp, "_", sc, "_sins.pdf"))
    ...
    if (all(file.exists(c(file_experts, file_kmeans, file_sins, file_greedy)))) {
      output_file <- file.path(out_dir, paste0("compare_", tp, "_", sc, ".pdf"))
