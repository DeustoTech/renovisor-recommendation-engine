...
clean_climate_zone <- function(x) {
  ...
  case_when(
    is.na(x) ~ NA_character_,
    str_detect(x, regex("Subtropical", ignore_case = TRUE)) ~ "Clima mediterráneo",
    str_detect(x, regex("Temperate Climates - Maritime", ignore_case = TRUE)) ~ "Clima oceánico",
    str_detect(x, regex("Temperate Climates - Continental|Temperate Climates - Intermediate", ignore_case = TRUE)) ~ "Clima continental",
    str_detect(x, regex("Transitional", ignore_case = TRUE)) ~ "Clima de transición",
...
clean_vote <- function(x) {
  ...
  case_when(
    ...
    str_detect(x, regex("do not vote|blank|null", ignore_case = TRUE)) ~ "No vota / blanco / nulo",
    ...
    str_detect(x, regex("abstain", ignore_case = TRUE)) ~ "Abstención",
    TRUE ~ "Otra respuesta"
  )
}
...
df <- df %>%
  mutate(
    ...
    year_of_birth_clean = case_when(
      !is.na(year_of_birth_raw) & year_of_birth_raw >= 1900 & year_of_birth_raw <= 2008 ~ year_of_birth_raw,
    ...
    age_group = case_when(
      year_of_birth_clean >= 2001 & year_of_birth_clean <= 2008 ~ "Generación Z",
...
sample_description <- df %>%
  transmute(
    ...
    residence_region = coalesce(
      clean_text_basic(region_from_dataset),
      clean_residence_region(country)
    ),
