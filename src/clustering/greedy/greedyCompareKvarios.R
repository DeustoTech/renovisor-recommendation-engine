...
read_binary_archetypes <- function(file_path, target_cols = NULL, top_n = NULL) {
  df <- fread(file_path)
  df <- as.data.frame(df, check.names = FALSE)
  ...
  # Si se pide top_n, dejar solo los top_n mayores por fila
  if (!is.null(top_n)) {
    ...
    for (i in seq_len(nrow(mat))) {
      ...
      keep <- ord[seq_len(min(top_n, sum(!is.na(x))))]
      out[i, keep] <- 1
    }
...
#out_root <- "results/det32/results_greedy_from_freq"
#out_root <- "results/det9/results_greedy_from_freq_9"
out_root <- "results/det9_top3/results_greedy_from_freq_9_top3"
...
expert_files <- list(
  #experts = "data/processed/dataPreproc/archetypeExperts_bin_32.csv"
...
for (sc in scenarios) {
  for (tp in types) {
    ...
    for (K in K_values) {
      ...
      k_dir <- file.path(case_dir, paste0("K_", K))
      ...
      for (D in D_values) {
        ...
        if (!is.null(centers_df)) {
          ...
          for (ref_name in names(expert_files)) {
            ...
            #expert_bin <- read_binary_archetypes(expert_file, target_cols = colnames(cluster_mat), top_n = 3)
            ...
            write.csv(coverage_df, file.path(dir_D, paste0("coverage_", ref_name, "_K", K, "_D", D, ".csv")), row.names = FALSE)
            ...
            write.csv(sim_df, file.path(dir_D, paste0("similarity_matrix_", ref_name, "_K", K, "_D", D, ".csv")), row.names = FALSE)
            ...
            write.csv(res_excl, file.path(dir_D, paste0("cluster_vs_", ref_name, "_exclusive_K", K, "_D", D, ".csv")), row.names = FALSE)
            
            matched_ok <- sum(!is.na(res_excl$best_match))
            mean_match <- mean(res_excl$pct_match, na.rm = TRUE)
            
            global_results[[paste(sc, tp, ref_name, K, D, sep = "_")]] <- data.frame(
              scenario = sc,
              type = tp,
              reference = ref_name,
              K = K,
      ...
      ggsave(file.path(k_dir, "assigned_vs_D_pct_lines.png"), plot = p, width = 8, height = 5)
    }
  }
}
...
if (length(pdf_results) > 0) {
  ...
  make_pdf_for_type <- function(df, type_value, outfile) {
    df_type <- df %>%
    ...
    p <- ggplot(df_type, aes(x = D, y = assigned_pct, color = K, group = K)) +
      geom_line(linewidth = 1) +
      geom_point(size = 1.8) +
      ...
      labs(
        title = paste("Greedy(3): % frecuencia asignada vs D para distintos K -", toupper(type_value)),
        x = "Distancia máxima D",
        y = "% frecuencia asignada",
        color = "K"
      ) +
  ...
  make_pdf_for_type(
    pdf_df,
    "pos",
    file.path(out_root, "assigned_vs_D_byK_pos.pdf")
  )
  ...
  for (tp in c("pos", "ext")) {
    ...
    for (sc in c("100-9-top3", "10-9-top3")) {
