...
# 4. Clasificar filas RV según una lógica agregada tipo TFG:
#    - ABSTENTION
#    - LEFT
#    - MODERATE
#    - CONSERVATIVE
...
#    - bloque votante según pesos externos agregados.
#
# Variante:
# Este script NO usa los grupos europeos detallados.
# Usa la clasificación agregada del TFG:
# LEFT / MODERATE / CONSERVATIVE / AUTHORITARIAN + ABSTENTION.
#
# Debe ejecutarse después de:
# 03_2_phase_dimension_scores
# y después del resumen de dataset.
...
out_dir <- file.path(
  processed_root,
  "04_3_propensity_bootstrap_tfg_blocks"
)
...
# En esta variante TFG, SWITCH_HANDLING no afecta a la clasificación final,
# porque los votantes se clasifican por escala izquierda-derecha.
...
# Clasificación electoral tipo TFG
...
classify_electoral_group <- function(vote_approach, lr, switch_handling = "national_by_lr") {
  ...
  case_when(
    vote_approach == "abstention" ~ "ABSTENTION",
    
    !is.na(lr) & lr < 40 ~ "LEFT",
    !is.na(lr) & lr >= 40 & lr < 60 ~ "MODERATE",
    !is.na(lr) & lr >= 60 & lr < 80 ~ "CONSERVATIVE",
    !is.na(lr) & lr >= 80 ~ "AUTHORITARIAN",
...
df_propensity <- df_propensity %>%
  mutate(
    ...
    electoral_group_quality = case_when(
      ...
      electoral_group_model %in% c("LEFT", "MODERATE", "CONSERVATIVE", "AUTHORITARIAN") ~ "classified_tfg_block",
...
# Tabla de targets electorales agregados tipo TFG

# Pesos dentro del bloque votante:
# Left          13.76
# Moderate      34.16
# Conservative  26.11
# Authoritarian 25.97
#
# Estos pesos suman 100 dentro de votantes.
# La abstención se fija por separado en TARGET_ABSTENTION_SHARE.
...
electoral_targets_voters_raw <- tribble(
  ...
  "LEFT",
  "Left",
  "voter with left-right position",
  "x < 40",
  13.76,
...
draw_one_bootstrap <- function(bootstrap_id, data, targets_voters) {
  ...
  # 2. Bloque votante por grupos agregados tipo TFG
  voter_pool_all <- data %>%
...
cat("\n============================================================\n")
cat("CALIDAD DE FILAS INCLUIDAS EN BOOTSTRAP\n")
cat("============================================================\n")
print(
  bootstrap_quality_flags_summary %>%
    group_by(
    ...
    summarise(
      mean_n = mean(n),
      mean_prop = mean(prop_within_type),
      .groups = "drop"
    ) %>%
    arrange(target_sample_type, desc(mean_n)),
  n = Inf,
